...
const listings = [
  {
    id: "sw-001",
    title: "Creative Office Tower — CBD",
  ...
  {
    id: "sw-002",
    title: "Modern Residence — Private Cluster",
  ...
  {
    id: "sw-003",
    title: "Mixed-Use Shophouse — Main Road",
...
app.get("/api/listings", (req, res) => {
  ...
  const filtered = listings.filter((item) => {
    const okQ = !q
      ? true
      : `${item.title} ${item.location}`.toLowerCase().includes(String(q).toLowerCase());
...
app.post("/api/contact", (req, res) => {
  ...
  if (!name || String(name).trim().length < 2) {
    return res.status(400).json({ ok: false, error: "Name minimal 2 karakter." });
  }
  ...
  if (!phoneOk) {
    return res.status(400).json({ ok: false, error: "Phone minimal 8 karakter angka/simbol." });
  }
  if (!message || String(message).trim().length < 10) {
    return res.status(400).json({ ok: false, error: "Message minimal 10 karakter." });
  }

  // Di production: simpan ke DB / kirim email (SendGrid/Mailgun) / CRM.
